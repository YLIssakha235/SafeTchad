...
export const Route = createFileRoute("/incidents/$incidentId")({
  loader: async ({ context, params }) => {
    try {
      ...
      // offline ou erreur réseau : le composant utilisera le cache éventuel
    }
  },
...
function getFriendlyErrorMessage(error: unknown) {
  ...
  if (
    ...
    return "Vous n’avez pas l’autorisation de modifier cet incident.";
  }

  return message || "Erreur lors de l’opération.";
}
...
function RouteComponent() {
  ...
  const canUploadByStatus = incident.status === "EN_COURS";

  const canUploadByPermission =
    (session?.user as { role?: string } | undefined)?.role === "ADMIN" ||
    session?.user?.id === incident.reporterId;

  const canUploadMedia = canUploadByStatus && canUploadByPermission;

  async function handleUpload() {
    if (!canUploadByStatus) {
      setUploadOk(false);
      setUploadMessage(
    ...
    if (!canUploadByPermission) {
      setUploadOk(false);
      setUploadMessage(
        "Vous ne pouvez pas ajouter d’image car vous n’êtes pas le créateur de cet incident."
      );
      return;
    }
    ...
    try {
      ...
      setUploadOk(false);
      setUploadMessage(getFriendlyErrorMessage(error));
    } finally {
    ...
    <div className="mx-auto max-w-3xl px-4 sm:px-6 py-8 sm:py-10 space-y-6 animate-fade-up">
      ...
      <div className="rounded-xl border bg-card overflow-hidden">
        ...
        <div className="grid grid-cols-1 gap-4 border-t border-border bg-muted/50 px-5 py-3 text-sm sm:grid-cols-2 lg:grid-cols-3 sm:px-6">
          ...
          <div>
            ...
            <p className="font-medium text-foreground">
              {formatLabel(incident.quartier ?? "Non précisé")}
            </p>
          </div>
          ...
          <div>
            ...
            <p className="font-medium text-foreground">
              {formatLabel(incident.axeRoutier ?? "Non précisé")}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-xl border bg-card p-5 sm:p-6 space-y-4 animate-fade-up delay-1">
        <div>
          ...
          <h2 className="font-semibold text-base">Position enregistrée</h2>
        </div>
        ...
        {incident.latitude != null && incident.longitude != null ? (
            ...
            <div className="rounded-lg border bg-muted/40 p-4">
              ...
              <p className="font-medium text-foreground">
                {incident.locationAccuracy != null
                  ? `±${incident.locationAccuracy.toFixed(0)} m`
                  : "Non précisée"}
              </p>
            </div>
          </div>
          ...
          <div className="rounded-lg border border-dashed border-border py-6 text-center text-sm text-muted-foreground">
            Aucune position GPS enregistrée pour cet incident.
          </div>
        )}
      </div>
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          ...
          {incident.medias && (
            <span className="rounded-full bg-muted px-2.5 py-1 text-xs text-muted-foreground">
              {incident.medias.length} fichier
              {incident.medias.length !== 1 ? "s" : ""}
            </span>
          )}
        </div>
      ...
      {canUploadMedia ? (
        ...
        <div className="rounded-xl border bg-card p-5 sm:p-6 text-sm text-muted-foreground animate-fade-up delay-2">
          {!canUploadByStatus
            ? "L’ajout d’image est désactivé car cet incident n’est plus en cours."
            : "Vous ne pouvez pas ajouter d’image car vous n’êtes pas le créateur de cet incident."}
        </div>

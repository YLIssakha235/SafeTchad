        ...
        {incidents.map((incident) => (
          <div key={incident.id} className="rounded-xl border p-5 shadow-sm space-y-3">
            ...
            <div className="grid gap-2 text-sm md:grid-cols-3">
              <div>
                <span className="font-medium">Ville :</span> {formatLabel(incident.ville)}
              </div>
              <div>
                <span className="font-medium">Quartier :</span>{" "}
                {incident.quartier ? formatLabel(incident.quartier) : "Non précisé"}
              </div>
              <div>
                <span className="font-medium">Axe routier :</span>{" "}
                {incident.axeRoutier ? formatLabel(incident.axeRoutier) : "Non précisé"}
              </div>
            </div>
          </div>
        ))}

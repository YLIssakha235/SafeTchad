...
function RouteComponent() {
  const { list: incidents, isLoading, error } = useIncidents(orpc);
        ...
        <div className="space-y-3">
          {incidents.map((incident, i) => {
            const type = TYPE_CONFIG[incident.type] ?? {
              label: incident.type, color: "text-muted-foreground", bg: "bg-muted", icon: "",
            };
                   ...
                   <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                    ...
                    <div className="flex items-center gap-4 text-xs text-muted-foreground pt-0.5 border-t border-border flex-wrap">
                      <span className="flex items-center gap-1">
                        ...
                        <span className="font-medium text-foreground">{formatLabel(incident.ville)}</span>
                      </span>
                      {incident.quartier && (
                        <span>· {formatLabel(incident.quartier)}</span>
                      )}
                      {incident.axeRoutier && (
                        <span>· {formatLabel(incident.axeRoutier)}</span>
                      )}
                      {"medias" in incident && Array.isArray(incident.medias) && incident.medias.length > 0 && (
